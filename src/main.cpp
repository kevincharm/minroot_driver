...
uint8_t* PrepareJob(Driver& driver, Model& model, size_t& size,
  ...
  driver.SerializeJob(buf, job_id, iteration_count + start_iter, start_iter, x, y);
  ...
  while ((opt = getopt(argc, argv, "t:f:v:i:s:n:e:h:x:y:")) != -1) {
    switch(opt) {
      ...
      case 'i':
        ...
        break;
      case 't':
        start_iter = (uint64_t)atol(optarg);
        printf("Command line start iter %" PRIu64 "\n", start_iter);
        break;
      case 's':
      ...
      for(unsigned i=0; i<num_engines && success; i++) {
        if (job[i]!=NULL) {
          ...
          while (success && worker[i]->GetResults(&success,&done)) {
            uint64_t cur_iter =
              (uint64_t)
                __builtin_bswap32(*(uint32_t*) &(buf[pvt + (i * size) + 4])) +
              ((uint64_t)
               __builtin_bswap32(*(uint32_t*) &(buf[pvt + (i * size) + 8]))
               << 32);
            printf("Intermediate %" PRId64 " on engine %d is %s\n",
                   cur_iter, i, success ?"OK":"NG");
            if (done) {
